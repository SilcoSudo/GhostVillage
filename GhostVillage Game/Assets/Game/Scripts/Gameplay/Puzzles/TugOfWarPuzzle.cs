...
///   2. Tạo Canvas UI (Screen Space Overlay) và assign vào tugOfWarCanvas
...
/// </summary>
public class TugOfWarPuzzle : MonoBehaviourPun, IInteractable
{
    [Header("--- Phần Thưởng ---")]
    ...
    public KeyItemSO keyItemReward;
        ...
        [SerializeField] private Canvas tugOfWarCanvas;
        ...
        [Header("--- Ông Kẹ ---")]
        [Tooltip("OngKeMonster trong scene – sẽ bị điều khiển khi thua")]
        [SerializeField] private OngKeMonster ongKe;
        [Tooltip("Thời gian Ông Kẹ đuổi sau khi thua (giây)")]
        [SerializeField] private float ongKeChaseDuration = 20f;
        ...
        private void StartLocalMinigame(GameObject actor)
        {
            ...
            // Mở UI
            if (tugOfWarCanvas != null) tugOfWarCanvas.gameObject.SetActive(true);
            ...
            Debug.Log("[KeoCoP] Bắt đầu kéo co!");
        }
        ...
        private void HandleLose()
        {
            ...
            // Báo MasterClient gọi OngKe đuổi người thua
            int loserActorNumber = PhotonNetwork.LocalPlayer.ActorNumber;
            photonView.RPC(nameof(AlertOngKeLoseRPC), RpcTarget.MasterClient, loserActorNumber);

            // Mở khóa bàn kéo co cho lần thử tiếp theo
            photonView.RPC(nameof(SetPlayingRPC), RpcTarget.All, false);

            Debug.Log("[KeoCoP] Người chơi THUA! Ông Kẹ sắp đến...");
        }
        ...
        private void CloseUI()
        {
            _isMinigamePlaying = false;
            _showingResult = false;
            if (tugOfWarCanvas != null) tugOfWarCanvas.gameObject.SetActive(false);
        }
        ...
        /// <summary>
        /// Master client nhận lệnh → báo OngKe đuổi người thua
        /// </summary>
        [PunRPC]
        private void AlertOngKeLoseRPC(int loserActorNumber)
        {
            if (!PhotonNetwork.IsMasterClient) return;
            if (ongKe == null)
            {
                Debug.LogWarning("[KeoCoP] Không tìm thấy OngKe reference!");
                return;
            }

            // Tìm Transform của người thua theo actor number
            foreach (var player in PhotonNetwork.PlayerList)
            {
                if (player.ActorNumber != loserActorNumber) continue;

                // Tìm GameObject của player đó trong scene
                foreach (var pv in FindObjectsOfType<PhotonView>())
                {
                    if (pv.Owner == player && pv.gameObject.CompareTag("Player"))
                    {
                        ongKe.ForceChasePlayer(pv.transform, ongKeChaseDuration);
                        Debug.Log($"[KeoCoP] OngKe đang đuổi Actor#{loserActorNumber} trong {ongKeChaseDuration}s");
                        return;
                    }
                }
            }
            Debug.LogWarning($"[KeoCoP] Không tìm thấy GameObject của Actor#{loserActorNumber}");
        }

        // ─── Khóa puzzle với tất cả client ───────────────────────
        [PunRPC]
        private void DisablePuzzleRPC()
        {
            _isSolved = true;
            _isAnyonePlaying = false;
            // Tắt Collider để không ai tương tác được nữa
            Collider col = GetComponent<Collider>();
            if (col != null) col.enabled = false;
            // Tắt UI phòng khi vẫn đang mở
            if (tugOfWarCanvas != null) tugOfWarCanvas.gameObject.SetActive(false);
