  ...
  const performSearch = async (searchQuery) => {
    ...
    try {
      const collectionsParam =
        activeTab === "all" ? filters.collections.join(",") : activeTab;
      ...
      setError(
        error.response?.data?.message || error.message || t("search.failed"),
      );
      setResults(null);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="search-page">
      <div className="search-header">
        <h1>
          {queryParam
            ? t("search.resultsTitleWithQuery", { query: queryParam })
          ...
          <div className="filters-panel">
            ...
            <div className="filter-options">
              ...
              <label>
                ...
                />
                {t("search.collections.announcements")}
              </label>
              <label>
                <input
                  type="checkbox"
      ...
      {results && (
        <div className="search-tabs">
          ...
          >
            {t("search.tabs.allResults", { count: results.totalResults })}
          </button>
          <button
          ...
          >
            <FileText size={16} />
            {t("search.tabs.posts", { count: getResultCount("posts") })}
          </button>
          <button
          ...
          >
            <BookOpen size={16} />
            {t("search.tabs.wiki", { count: getResultCount("wiki") })}
          </button>
          <button
          ...
          >
            <Megaphone size={16} />
            {t("search.tabs.announcements", {
              count: getResultCount("announcements"),
            })}
          </button>
          <button
          ...
          >
            <User size={16} />
            {t("search.tabs.users", { count: getResultCount("users") })}
          </button>
        </div>
      )}
      ...
      <div className="search-results">
        {error && (
          <div className="error-message">
            ...
            <p>{error}</p>
            <p className="error-hint">{t("search.errorHint")}</p>
          </div>
        )}
        ...
        {!loading && results && (
          <>
            ...
            {results.results.posts &&
              results.results.posts.items.length > 0 && (
                <div className="results-section">
                  <h2 className="section-title">
                    <FileText size={20} />
                    {t("search.section.posts", {
                      count: results.results.posts.total,
                    })}
                  </h2>
            ...
            {results.results.wiki && results.results.wiki.items.length > 0 && (
              <div className="results-section">
                <h2 className="section-title">
                  <BookOpen size={20} />
                  {t("search.section.wiki", {
                    count: results.results.wiki.total,
                  })}
                </h2>
                <div className="results-list">
                  {results.results.wiki.items.map((wiki) => (
                    ...
                    >
                      ...
                      <div className="result-content">
                        ...
                        {wiki.excerpt && (
                          <p className="result-excerpt">{wiki.excerpt}</p>
                        )}
                        <div className="result-meta">
                          <span className="category-badge">
                            {wiki.category}
                          ...
                          <span>
                            <Eye size={14} />
                            {t("search.views", { count: wiki.views })}
                          </span>
                        </div>
                      </div>
                    </Link>
            ...
            {results.results.announcements &&
              results.results.announcements.items.length > 0 && (
                <div className="results-section">
                  <h2 className="section-title">
                    <Megaphone size={20} />
                    {t("search.section.announcements", {
                      count: results.results.announcements.total,
                    })}
                  </h2>
                  <div className="results-list">
                      ...
                      >
                        ...
                        <div className="result-content">
                          ...
                          <div className="result-meta">
                            ...
                            <span>
                              <Eye size={14} />
                              {t("search.views", { count: announcement.views })}
                            </span>
                          </div>
                        </div>
                      </Link>
                    ...
                    {results.results.users.items.map((user) => (
                      ...
                      >
                        <img
                          src={
                            user.avatar ||
                            `https://ui-avatars.com/api/?name=${user.fullname || user.username}&background=f48024&color=fff`
                          }
